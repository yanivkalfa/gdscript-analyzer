...
fn dispatch(cli: &Cli) -> anyhow::Result<i32> {
    let g = &cli.global;
    match &cli.command {
        Command::Check(p) => run_diagnostics(p, g, DiagFilter::All),
        Command::Lint(p) => run_diagnostics(p, g, DiagFilter::WarningsOnly),
        Command::Symbols(p) => run_symbols(p, g),
        Command::Format(args) => Ok(run_format(args, g)),
    }
}

/// `check` / `lint`: load the project, run diagnostics in parallel, emit, and pick the exit code.
fn run_diagnostics(paths: &PathsArg, g: &GlobalArgs, filter: DiagFilter) -> anyhow::Result<i32> {
    ...
    let failing = results
        ...
        .any(|d| is_failing(d.severity, g.error_on_warning));
